...
def bellman_loss(p_model, p_model_state, t_model, t_model_state, obs, actions, next_obs, rewards, dones, gamma, v_min, v_max):
    ...
    def RKHSloss(next_atoms, next_pmfs, p_atoms, pmfs, p_sig):
        ...
        term1 = (v_Gram(v_p_atoms, next_atoms, p_sig)*(jnp.expand_dims(pmfs,-1)@jnp.expand_dims(next_pmfs,1))).sum()
        term2 = (Gram(p_atoms, p_atoms, p_sig)*(pmfs.T@pmfs)).sum()
        term0 = (v_Gram(next_atoms, next_atoms, p_sig)*(jnp.expand_dims(next_pmfs,-1)@jnp.expand_dims(next_pmfs,1))).sum()
        loss = term0 -2*term1 + term2
        return loss/next_pmfs.shape[0]
    
    loss = RKHSloss(jnp.expand_dims(next_atoms,-1), next_pmfs, jnp.expand_dims(p_atoms,-1), pmfs, p_sig)
...
@hydra.main(config_name='Proposal_Single', version_base=None, config_path="config")
def main(cfg):
    #managers, env
    ...
    v_sig=jnp.broadcast_to(cfg.model.sig, (cfg.num_env,1))

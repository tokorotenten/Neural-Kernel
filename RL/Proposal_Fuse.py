...
from model.NKME_models import NN as NN
...
def get_distance(X, Y):
  def pairwisedist(X,Y):
    ...
    return jax.vmap(vmapped_dist, in_axes=(None, 0))(X,Y)
  S = pairwisedist(X, Y).T
  S = S[jnp.triu_indices(S.shape[0])]
  return S
...
def policy(model, model_state, obs, action_size, eps, key):
    ...
    def greedy(subkey):
        pmfs, state, atoms, sig = model(obs, model_state)
...
def bellman_loss(p_model, p_model_state, t_model, t_model_state, obs, actions, next_obs, rewards, dones, gamma, v_min, v_max):
    t_batch_model = jax.vmap(t_model, in_axes=(0, None), out_axes=(0, None, None, None))
    p_model = eqx.nn.inference_mode(p_model, False)
    p_batch_model = jax.vmap(p_model, in_axes=(0, None), out_axes=(0, None, None, None))

    next_pmfs, t_model_state, t_atoms, t_sig = t_batch_model(next_obs, t_model_state)  # (batch_size, num_actions, num_atoms)
    ...
    pmfs, p_model_state, p_atoms, p_sig = p_batch_model(obs, p_model_state)
    pmfs = pmfs[jnp.arange(pmfs.shape[0]), actions.astype(int)]

    def RKHSloss(next_atoms, next_pmfs, p_atoms, pmfs, p_sig):
        v_Gram = jax.vmap(Gram, in_axes=(0, 0, None))
        v_p_atoms = jnp.broadcast_to(p_atoms, (next_atoms.shape[0], p_atoms.shape[0], p_atoms.shape[1]))
        term1 = (v_Gram(v_p_atoms, next_atoms, p_sig)*(jnp.expand_dims(pmfs,-1)@jnp.expand_dims(next_pmfs,1))).sum((1,2))
        term2 = (pmfs@Gram(p_atoms, p_atoms, p_sig)@pmfs.T).diagonal()
        term0 = (v_Gram(next_atoms, next_atoms, p_sig)*(jnp.expand_dims(next_pmfs,-1)@jnp.expand_dims(next_pmfs,1))).sum((1,2))
        loss = term0 -2*term1 + term2
        return loss
    
    v_RKHSloss = jax.vmap(RKHSloss, in_axes=(None, None, None, None, 0))
    loss = logsumexp(v_RKHSloss(jnp.expand_dims(next_atoms,-1), next_pmfs, jnp.expand_dims(p_atoms,-1), pmfs, p_sig), axis=0, b=1/p_sig.shape[0])
    loss = loss.mean()
...
@hydra.main(config_name='Proposal_Fuse', version_base=None, config_path="config")
def main(cfg):
    #managers, env
    ...
    yc = jnp.array([np.linspace(cfg.model.v_min, cfg.model.v_max, cfg.model.numAtom)]).T
    S = get_distance(yc, yc)
    SS = jnp.sort(S)
    sig = jnp.linspace(SS[jnp.floor(len(SS)*0.05).astype(int)]/2, SS[jnp.floor(len(SS)*0.95).astype(int)]/2, cfg.model.mix)
    sig = jnp.array([sig]).T
    v_sig=jnp.broadcast_to(sig, (cfg.num_env, cfg.model.mix, 1))

    p_model, p_model_state = eqx.filter_vmap(eqx.nn.make_with_state(NN))(rollout_manager.observation_space, cfg.model.numAtom*rollout_manager.action_size, v_atoms, v_sig,  mkeys)
    t_model = p_model

...
@hydra.main(config_name='toy_NF', version_base=None, config_path="config")
def main(cfg):
  ...
  with mlflow.start_run():
    ...
    for i in tqdm(range(vX.shape[0])):
       ...
       for j in range(cfg.model.num_flows):
        flows += [nf.flows.AutoregressiveRationalQuadraticSpline(1, cfg.model.hidden_layers, cfg.model.hidden_units, num_context_channels=1)]
       ...
       was[i] = wasserstein(yp, samples).mean()
